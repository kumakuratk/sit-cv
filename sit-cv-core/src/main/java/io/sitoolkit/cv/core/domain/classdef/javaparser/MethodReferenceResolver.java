...
class MethodReferenceResolver {
  ...
  public Optional<ResolvedMethodDeclaration> resolve(MethodReferenceExpr methodRefExpr) {
    ...
    Optional<ResolvedMethodDeclaration> result =
        findMethodDeclation(resolveMethodsFromTypeOf(scope), identifier);
  ...
  Optional<ResolvedMethodDeclaration> findMethodDeclation(
    ...
    if (found.size() == 1) {
      ...
      log.debug(
          "Coudn't specify method reference:'{}' because type has overloaded methods: {}",
          methodIdentifier,
          found);
      return Optional.empty();
